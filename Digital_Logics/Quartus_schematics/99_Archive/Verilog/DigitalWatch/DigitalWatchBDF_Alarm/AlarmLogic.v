`timescale 1ns / 1ps
module AlarmLogic(reset, clk100hz, mode, set_pos, clk_min, clk_hour, sw2, 
						sw3, sec_out, min_out, hour_out, alarm_out, alarm_on);
						
	input				reset, clk100hz;
	input [1:0]		mode;
	input [2:0] 	set_pos;
	input [5:0] 	clk_min;
	input [4:0] 	clk_hour;
...
// always for second
	always @(posedge(sw2) or negedge(reset)) begin
		if (~reset)
			sec <= 6'd0;
		else
