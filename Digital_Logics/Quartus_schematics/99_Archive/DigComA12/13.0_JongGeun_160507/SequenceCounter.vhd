...
ENTITY SequenceCounter IS
	...
	--	inst_step_clk_out		:	out std_logic;
	...
	--	sc_clk_out		: out	std_logic;
		--ClockStepMode_out			: out std_logic; --m0 and m1 : 01
		InstructionStepMode_out	: out std_logic --m0 and m1 : 10
	   --RunMode_out					: out std_logic -- m0 and m1 : 11
	);
END SequenceCounter;
...
--	signal	RunMode					: std_logic; -- m0 and m1 : 00, 11
...
-- stop_node ==> 0 : stop, 1 : run
	hlt_stop_out <= hlt_stop;
	stop_node <= hlt_stop and ((not inst_stop) or (not instructionStepMode));
--	start_stop <= stop_node;
--	start_stop <= hlt_stop and ((not inst_stop) or (not instructionStepMode));
--	start_stop <= hlt_stop;
...
-- sequence counter clock
 ...
 --	state_mode_out(2) <= runMode;
--	state_mode_out(1) <= instructionStepMode;
--	state_mode_out(0) <= clockStepMode;
